...
import (
	...
	docs "github.com/blinklabs-io/bursa/docs" // docs is generated by Swag CLI
...
type AddressParseResponse struct {
	...
	Payment  *CredentialInfo   `json:"payment,omitempty"`
	Stake    *CredentialInfo   `json:"stake,omitempty"`
	Pointer  *PointerInfo      `json:"pointer,omitempty"`
	Byron    *ByronAddressInfo `json:"byron,omitempty"`
}

// CredentialInfo defines information about a credential (key hash or script hash)
type CredentialInfo struct {
	Type   string `json:"type"`   // "key" or "script"
	Bech32 string `json:"bech32"` // bech32-encoded credential
	Hex    string `json:"hex"`    // hex-encoded credential
}

// PointerInfo defines information about a stake pointer
...
// ByronAddressInfo defines information about a Byron address
type ByronAddressInfo struct {
	Type string `json:"type"` // "pubkey", "script", or "redeem"
}

// AddressBuildRequest defines the request payload for address building
...
// AddressBuildResponse defines the response payload for address building
type AddressBuildResponse struct {
	Address string `json:"address"`
	Network string `json:"network"`
	Type    string `json:"type"`
}
...
func apiTLSConfigured(cfg *config.Config) (bool, error) {
	...
	hasCert := cfg.Api.TLSCertFile != ""
	hasKey := cfg.Api.TLSKeyFile != ""
	if hasCert != hasKey {
		return false, errors.New("api.tls_cert_file and api.tls_key_file must both be configured")
	}
	return hasCert, nil
}
...
func validateAPIExposure(cfg *config.Config, auth signerapi.Validator) error {
	...
	tlsConfigured, err := apiTLSConfigured(cfg)
	if err != nil {
		return err
	}
	if !signer.IsLoopbackListenAddress(cfg.Api.ListenAddress) && !tlsConfigured {
		return fmt.Errorf(
			"refusing plaintext API listener on non-loopback address %q: configure TLS with api.tls_cert_file and api.tls_key_file",
			cfg.Api.ListenAddress,
		)
	}
...
func configureSwaggerScheme(tlsConfigured bool) {
	if tlsConfigured {
...
func Start(
	...
	configureSwaggerScheme(tlsConfigured)
...
func handleWalletList(w http.ResponseWriter, r *http.Request) {
	...
	_, _ = w.Write(resp) //nolint:gosec // G705: Content-Type is set to application/json above
}
...
// handleAddressParse godoc
//
//	@Summary		Parse Cardano address
//	@Description	Parses a Cardano address and returns its components and metadata
//	@Accept			json
//	@Produce		json
//	@Param			request	body		AddressParseRequest		true	"Address Parse Request"
//	@Success		200		{object}	AddressParseResponse	"Address parsed successfully"
//	@Failure		400		{object}	ErrorResponse			"Invalid request or address"
//	@Failure		500		{object}	ErrorResponse			"Internal server error"
//	@Router			/api/address/parse [post]
func handleAddressParse(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodPost {
		http.Error(w, "Method not allowed", http.StatusMethodNotAllowed)
		return
	}
	...
	if err != nil {
		...
		writeJSONError(
			w,
			logger,
			map[string]string{"error": "Failed to parse address"},
	...
	resp, err := json.Marshal(response)
	if err != nil {
		logger.Error("failed to serialize address parse response", "error", err)
		w.Header().Set("Content-Type", "application/json")
		w.WriteHeader(http.StatusInternalServerError)
		_, _ = w.Write([]byte(`{"error":"Internal server error"}`))
		return
...
// handleAddressBuild godoc
//
//	@Summary		Build Cardano address
//	@Description	Builds a Cardano address from verification keys
//	@Accept			json
//	@Produce		json
//	@Param			request	body		AddressBuildRequest		true	"Address Build Request"
//	@Success		200		{object}	AddressBuildResponse	"Address built successfully"
//	@Failure		400		{object}	ErrorResponse			"Invalid request or keys"
//	@Failure		500		{object}	ErrorResponse			"Internal server error"
//	@Router			/api/address/build [post]
func handleAddressBuild(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodPost {
		http.Error(w, "Method not allowed", http.StatusMethodNotAllowed)
		return
	}
	...
	// Build the address
	address, addressType, err := buildAddressFromKeys(req)
	if err != nil {
		logger.Error("failed to build address", "error", err)
		writeJSONError(
			w,
			logger,
			map[string]string{"error": err.Error()},
	...
	resp, err := json.Marshal(response)
	if err != nil {
		logger.Error("failed to serialize address build response", "error", err)
		w.Header().Set("Content-Type", "application/json")
		w.WriteHeader(http.StatusInternalServerError)
		_, _ = w.Write([]byte(`{"error":"Internal server error"}`))
		return
...
func buildAddressParseResponse(
	addr *lcommon.Address,
	addressStr string,
) (*AddressParseResponse, error) {
	...
	if addrType == lcommon.AddressTypeByron {
		...
		response.Byron = &ByronAddressInfo{
			Type: byronTypeStr,
		}
		return response, nil
	}
	...
	if hasPayment {
		...
		bech32Str, hexStr, err := formatCredential(
			paymentHash[:],
			paymentIsScript,
			true,
		)
		if err != nil {
			return nil, fmt.Errorf(
				"failed to format payment credential: %w",
				err,
			)
		}
		response.Payment = &CredentialInfo{
			Type:   credType,
			Bech32: bech32Str,
			Hex:    hexStr,
		}
	...
	if hasStake && !isPointer {
		...
		if err != nil {
			return nil, fmt.Errorf("failed to format stake credential: %w", err)
		}
		response.Stake = &CredentialInfo{
			Type:   credType,
			Bech32: bech32Str,
			Hex:    hexStr,
		}
	...
	// Add pointer information (placeholder for now)
	if isPointer {
		response.Pointer = &PointerInfo{
...
// buildAddressFromKeys builds an address from the provided keys
func buildAddressFromKeys(req AddressBuildRequest) (string, string, error) {
	// Validate network
	network, ok := ouroboros.NetworkByName(req.Network)
	if !ok {
		return "", "", fmt.Errorf("invalid network: %s", req.Network)
	}

	// Parse keys based on address type
	switch req.Type {
	case "base":
		if req.PaymentKey == "" || req.StakeKey == "" {
			return "", "", errors.New(
				"base addresses require both payment and stake keys",
			)
		}
		paymentHash, err := parseVerificationKey(req.PaymentKey, true)
		if err != nil {
			return "", "", fmt.Errorf("invalid payment key: %w", err)
		}
		stakeHash, err := parseVerificationKey(req.StakeKey, false)
		if err != nil {
		...
		return addr.String(), "base", nil

	case "enterprise":
		if req.PaymentKey == "" {
			return "", "", errors.New(
				"enterprise addresses require a payment key",
			)
		}
		paymentHash, err := parseVerificationKey(req.PaymentKey, true)
		if err != nil {
		...
		return addr.String(), "enterprise", nil
	...
	case "reward":
		...
		return addr.String(), "reward", nil

	default:
		return "", "", fmt.Errorf("unsupported address type: %s", req.Type)
	}
}

// parseVerificationKey parses a bech32-encoded verification key and returns its hash
func parseVerificationKey(keyStr string, isPayment bool) ([28]byte, error) {
	var hash [28]byte
...
// formatCredential formats a credential hash in both bech32 and hex formats
func formatCredential(
	hash []byte,
	isScript, isPayment bool,
) (string, string, error) {
