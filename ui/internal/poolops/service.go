...
// Tipper reports the node's current tip slot (from the supervisor status).
...
// SetAccount records the active wallet account so pool operations can run.
...
// ErrWrongPassword.
func (s *Service) unlockRoot(password string) (bip32.XPrv, func(), error) {
	if s.keys == nil {
...
func (s *Service) Credentials(password string) (Credentials, error) {
	...
	root, zero, err := s.unlockRoot(password)
	if err != nil {
...
func (s *Service) resolveRewardAccount(p RegistrationParams, acct *wallet.Account) ([]byte, error) {
	...
	b, err := addr.Bytes()
	if err != nil {
		return nil, fmt.Errorf("reward address bytes: %w", err)
	}
	return b, nil
}
...
func (s *Service) buildRegistration(coldVKey, vrfHash []byte, p RegistrationParams, acct *wallet.Account) (CertResult, error) {
	...
	if u := strings.TrimSpace(p.MetadataURL); u != "" {
		hb, err := hex.DecodeString(strings.TrimSpace(p.MetadataHash))
		if err != nil {
...
func (s *Service) SubmitRetirement(ctx context.Context, password string, epoch uint64) (TxResult, error) {
	...
	for _, addrStr := range acct.ReceiveAddresses {
		...
		if len(utxos) > 0 {
			...
			for _, u := range utxos {
				ref := hex.EncodeToString(u.Id.Id().Bytes()) + "#" + fmt.Sprintf("%d", u.Id.Index())
	...
	for _, inp := range tx.Body.TxInputs.Items() {
		ref := hex.EncodeToString(inp.TxId.Bytes()) + "#" + fmt.Sprintf("%d", inp.OutputIndex)
		addrStr, ok := utxoAddr[ref]
		if !ok || seen[addrStr] {
			continue
		}
		seen[addrStr] = true
		...
		addr, err := lcommon.NewAddress(addrStr)
		if err != nil {
			return TxResult{}, fmt.Errorf("parse address %s: %w", addrStr, err)
		}
		kw, err := apollo.NewKeyPairWallet(addr, payKey)
		if err != nil {
			return TxResult{}, fmt.Errorf("key pair wallet idx %d: %w", idx, err)
		}
		a = a.SetWallet(kw)
		a, err = a.Sign()
		for i := range payKey {
		...
		if err != nil {
			return TxResult{}, fmt.Errorf("sign idx %d: %w", idx, err)
		}
	}
