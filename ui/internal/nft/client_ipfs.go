...
// MediaAvailable reports whether this binary includes the embedded IPFS
// client.
func MediaAvailable() bool { return true }

// blockCacheBytes bounds all transient bitswap block data in memory while
// leaving enough room for several maximum-size NFT images to be assembled.
const blockCacheBytes = 32 << 20 // 32 MiB

// maxIPFSBlockBytes is the block-size ceiling specified by bitswap. Enforcing
// it again at the datastore boundary prevents an oversized block from ever
// becoming retained cache data.
const maxIPFSBlockBytes = 2 << 20 // 2 MiB
...
func newIPFSClient(ctx context.Context) (fetcher, error) {
	...
	blockDS, err := newLRUDatastore(blockCacheBytes, maxIPFSBlockBytes)
	if err != nil {
		_ = kad.Close()
		_ = h.Close()
		return nil, fmt.Errorf("nft: block datastore: %w", err)
	}
	ds := dssync.MutexWrap(blockDS)
	bs := blockstore.NewBlockstore(ds)
	net := bsnet.NewFromIpfsHost(h)
	// This wallet is a retrieval client, not an IPFS provider. Bitswap enables
	// its serving side by default, which would otherwise answer peer requests
	// from the same blockstore populated by NFT fetches.
	bswap := bitswap.New(
		ctx,
		net,
		kad,
