...
func (s *Service) saveEnabled(enabled bool) error {
	...
	return os.WriteFile(s.setPath, b, 0o600)
}
...
func (s *Service) SetEnabled(enabled bool) error {
	...
	if enabled {
		if err := s.startClientLocked(); err != nil {
			// Roll back the in-memory flag but keep the persisted intent: the
			// user asked for it; a transient start failure shouldn't silently
			// flip the setting back off on disk.
			s.enabled = false
			return err
		}
	} else {
...
// imageForUnit resolves the image CID for a wallet-held asset unit by querying
// the node's asset metadata. Returns "" (no error) when the asset has no
// IPFS image. It first confirms the active wallet actually holds the unit, so a
// crafted request can't drive the embedded client to fetch arbitrary CIDs.
func (s *Service) imageForUnit(ctx context.Context, unit string) (string, error) {
	...
	if !ok {
		// Only serve images for assets the active wallet actually holds.
		return "", ErrNoWallet
	}
	info, err := s.chain.Asset(ctx, unit)
	if err != nil {
	...
	return parseMetadata(info.OnchainMetadata).ImageCID, nil
}
