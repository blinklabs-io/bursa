...
// the wallet's identity model forbids. UnixFS paths are also rejected until the
// fetcher can resolve them; treating cid/path as cid would fetch the wrong
// object. Returns "" when no supported, valid CID is found.
func parseImageCID(uri string) string {
	...
	// Accept both common spellings of the redundant /ipfs/ namespace prefix.
	s = strings.TrimPrefix(s, "/ipfs/")
	s = strings.TrimPrefix(s, "ipfs/")
	// The fetch surface currently accepts only a root CID. Do not silently
	// discard a UnixFS path and fetch a different object at the DAG root.
