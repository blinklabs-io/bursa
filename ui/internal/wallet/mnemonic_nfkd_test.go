...
// TestRootKeyFromMnemonicBytesFormInvariant covers the derivation entry point:
// both Unicode forms of one phrase must produce the same root key.
func TestRootKeyFromMnemonicBytesFormInvariant(t *testing.T) {
	useJapaneseWordList(t)
	...
	want, err := RootKeyFromMnemonicBytes([]byte(nfkdMnemonic))
	if err != nil {
		t.Fatalf("RootKeyFromMnemonicBytes(NFKD): %v", err)
	}
	got, err := RootKeyFromMnemonicBytes([]byte(nfcMnemonic))
	if err != nil {
		t.Fatalf("RootKeyFromMnemonicBytes(NFC): %v", err)
	}
	if !bytes.Equal(got, want) {
		t.Fatalf("root key differs between NFC and NFKD input")
	}
}
...
func TestMnemonicNormalizationRejectsInvalid(t *testing.T) {
	...
	if _, err := entropyFromMnemonicBytes([]byte(broken)); !errors.Is(err, bursa.ErrInvalidMnemonic) {
		t.Fatalf("err = %v, want ErrInvalidMnemonic", err)
	}
	if _, err := entropyFromMnemonicBytes([]byte("\xff\xfe not utf8 at all")); !errors.Is(err, bursa.ErrInvalidMnemonic) {
