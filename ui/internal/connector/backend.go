...
type WalletBackend struct {
	...
	// mu guards acct, which is rebound whenever the active wallet changes
	// (unlock/activate/add). dApp request handlers read it concurrently with the
	// API server's lifecycle handlers, so all access goes through account()/
	// SetAccount.
	mu   sync.RWMutex
	acct *wallet.Account
}
...
func (b *WalletBackend) SetAccount(acct *wallet.Account) {
	b.mu.Lock()
	b.acct = acct
	b.mu.Unlock()
}
...
func (b *WalletBackend) SignTx(ctx context.Context, txHex string, partialSign bool, password string) (string, error) {
	...
	wsCbor, err := b.sp.WitnessTx(bodyCbor, requiredSigners, inputAddrs, password, partialSign)
	if err != nil {
		return "", err
	}
