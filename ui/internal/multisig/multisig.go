...
// the native script, so the headroom matters more here.
...
func (s *Service) Build(ctx context.Context, id string, req BuildRequest) (UnsignedTx, error) {
	...
	// Attach the native script so the witness set carries it (the ledger needs the
	// script that hashes to the input's payment credential), and register the
	// participant key-hashes as required signers so Apollo's fee estimate accounts
	// for the witnesses that will be added during collection.
	...
	for _, kh := range keyHashes {
		var b224 lcommon.Blake2b224
		copy(b224[:], kh)
		a = a.AddRequiredSigner(b224)
	}
