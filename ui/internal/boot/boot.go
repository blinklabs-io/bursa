...
func Boot(ctx context.Context, cfg Config) (*App, error) {
	...
	// client verifies pasted pool/DRep IDs, reads protocol params for the staking
	// flow, and serves genesis for the SPO KES-period math (consent law: the
	// embedded node is the only network contact).
	...
	// Spending builds/signs/submits through the node's loopback UTxO-RPC endpoint;
	// the active wallet's seed is decrypted from the vault on demand. Delegation
	...
	// pool certificates on the active wallet. Genesis (for KES-period math) comes
	// from the node's loopback Blockfrost endpoint; the tip comes from the
	// supervisor — no external call.
	poolSvc := poolops.NewService(
		chainCtx, vaultKeystore{v: vlt},
		genesisAdapter{c: chainClient},
		tipAdapter{sup: sup},
	)
	...
	srv := &http.Server{
		Handler: api.NewHandler(
			sup, vlt, walletSvc, spendSvc, chainClient, poolSvc, multisigSvc, dexSvc,
...
// vaultKeystore adapts the encrypted vault to the spend/poolops/multisig
// services' Keystore interface. The vault owns seed encryption and the
// active-wallet selection, so Unlock decrypts the ACTIVE wallet's seed under the
// supplied spending password; Create is unsupported (wallets are added via the
// vault) and Exists is true whenever a wallet is active.
