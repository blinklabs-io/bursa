...
// TestNewHandlerConnectorWiring tests the opt-in connector gate in NewHandler.
func TestNewHandlerConnectorWiring(t *testing.T) {
	st := fakeStatuser{s: supervisor.Status{State: supervisor.StateReady}}

	t.Run("with cn != nil: POST /connector/pair is reachable (not 404)", func(t *testing.T) {
		cn := connector.NewService(t.TempDir(), &fakeConnectorBackend{}, nil)
		h := NewHandler(st, &fakeVault{}, &fakeWallet{}, &fakeSpender{}, &fakeSettings{}, cn, "preview", http.NotFoundHandler())

		body := bytes.NewBufferString(`{"extension_id":"chrome-extension://test","code":""}`)
		rec := httptest.NewRecorder()
		h.ServeHTTP(rec, httptest.NewRequest(http.MethodPost, "/connector/pair", body))
		if rec.Code == http.StatusNotFound {
			t.Fatalf("POST /connector/pair with cn set = 404, want not-404 (got %d)", rec.Code)
		}
	})

	t.Run("with cn != nil: POST /connector/request without token → 401 (token middleware applies)", func(t *testing.T) {
		cn := connector.NewService(t.TempDir(), &fakeConnectorBackend{}, nil)
		h := NewHandler(st, &fakeVault{}, &fakeWallet{}, &fakeSpender{}, &fakeSettings{}, cn, "preview", http.NotFoundHandler())

		body := bytes.NewBufferString(`{"origin":"https://a.io","method":"getBalance","params":null}`)
		req := httptest.NewRequest(http.MethodPost, "/connector/request", body)
		req.Header.Set("Origin", "chrome-extension://test")
		// No X-Bursa-Token header.
		rec := httptest.NewRecorder()
		h.ServeHTTP(rec, req)
		if rec.Code != http.StatusUnauthorized {
			t.Fatalf("POST /connector/request without token = %d, want 401", rec.Code)
		}
	})

	t.Run("with cn != nil: GET /connector/grants from same-origin (no Origin) → 200 (NOT token-gated)", func(t *testing.T) {
		cn := connector.NewService(t.TempDir(), &fakeConnectorBackend{}, nil)
		h := NewHandler(st, &fakeVault{}, &fakeWallet{}, &fakeSpender{}, &fakeSettings{}, cn, "preview", http.NotFoundHandler())

		rec := httptest.NewRecorder()
		// No Origin header → same-origin.
		h.ServeHTTP(rec, httptest.NewRequest(http.MethodGet, "/connector/grants", nil))
		if rec.Code != http.StatusOK {
			t.Fatalf("GET /connector/grants (same-origin, no token) = %d, want 200", rec.Code)
		}
	})

	t.Run("with cn != nil: POST /connector/decide is reachable (not routing-404)", func(t *testing.T) {
		cn := connector.NewService(t.TempDir(), &fakeConnectorBackend{}, nil)
		h := NewHandler(st, &fakeVault{}, &fakeWallet{}, &fakeSpender{}, &fakeSettings{}, cn, "preview", http.NotFoundHandler())

		// A malformed JSON body → 400 proves the route is registered (a routing
		// 404 would never reach the handler to parse the body).
		body := bytes.NewBufferString("{bad json")
		rec := httptest.NewRecorder()
		h.ServeHTTP(rec, httptest.NewRequest(http.MethodPost, "/connector/decide", body))
		if rec.Code == http.StatusNotFound {
			t.Fatalf("POST /connector/decide with cn set = 404 (routing miss), want non-404 (got %d)", rec.Code)
		}
	})

	t.Run("with cn == nil: POST /connector/pair → 404 (connector not registered)", func(t *testing.T) {
		h := NewHandler(st, &fakeVault{}, &fakeWallet{}, &fakeSpender{}, &fakeSettings{}, nil, "preview", http.NotFoundHandler())
		...
		h.ServeHTTP(rec, httptest.NewRequest(http.MethodPost, "/connector/pair", body))
		if rec.Code != http.StatusNotFound {
			t.Fatalf("POST /connector/pair with cn=nil = %d, want 404", rec.Code)
		}
	})
}
