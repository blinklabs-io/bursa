...
func TestStatusReturnsSnapshot(t *testing.T) {
	...
	h := NewHandler(fakeStatuser{s: want}, &fakeVault{}, &fakeWallet{}, &fakeSpender{}, nil, &fakePoolOps{}, &fakeMultiSig{}, &fakeDexQuoter{}, &fakeSettings{}, "preview", http.NotFoundHandler())
	rec := httptest.NewRecorder()
...
func TestVaultStatusReports(t *testing.T) {
	...
	h := NewHandler(fakeStatuser{}, fv, &fakeWallet{}, &fakeSpender{}, nil, &fakePoolOps{}, &fakeMultiSig{}, &fakeDexQuoter{}, &fakeSettings{}, "preview", http.NotFoundHandler())
	rec := httptest.NewRecorder()
...
func TestVaultStatusReportsLegacyKeystore(t *testing.T) {
	...
	h := NewHandler(fakeStatuser{}, fv, &fakeWallet{}, &fakeSpender{}, nil, &fakePoolOps{}, &fakeMultiSig{}, &fakeDexQuoter{}, &fakeSettings{}, "preview", http.NotFoundHandler(), WithLegacyKeystore(legacy))
	rec := httptest.NewRecorder()
	h.ServeHTTP(rec, httptest.NewRequest(http.MethodGet, "/vault/status", nil))
	if rec.Code != http.StatusOK {
		t.Fatalf("GET /vault/status = %d, want 200", rec.Code)
...
func TestVaultUnlockBindsSoleWalletAndReads(t *testing.T) {
	...
	h := NewHandler(st, fv, fw, &fakeSpender{}, nil, &fakePoolOps{}, &fakeMultiSig{}, &fakeDexQuoter{}, &fakeSettings{}, "preview", http.NotFoundHandler())
...
func TestVaultLock(t *testing.T) {
	...
	h := NewHandler(fakeStatuser{}, fv, fw, sp, nil, &fakePoolOps{}, &fakeMultiSig{}, &fakeDexQuoter{}, &fakeSettings{}, "preview", http.NotFoundHandler())
	rec := httptest.NewRecorder()
...
func TestMigrateLegacyKeystoreImportsAndBinds(t *testing.T) {
	...
	h := NewHandler(fakeStatuser{}, fv, fw, sp, nil, &fakePoolOps{}, &fakeMultiSig{}, &fakeDexQuoter{}, &fakeSettings{}, "preview", http.NotFoundHandler(), WithLegacyKeystore(legacy))
...
func TestAddWalletEnablesReadsAndSpend(t *testing.T) {
	...
	fw := &fakeWallet{balance: wallet.Balance{Lovelace: "1234"}}
	sp := &fakeSpender{}
	h := NewHandler(st, fv, fw, sp, nil, &fakePoolOps{}, &fakeMultiSig{}, &fakeDexQuoter{}, &fakeSettings{}, "preview", http.NotFoundHandler())
...
func TestAddWalletRequiresSpendPassword(t *testing.T) {
	...
	fv := &fakeVault{exists: true, locked: false}
	sp := &fakeSpender{}
	h := NewHandler(st, fv, &fakeWallet{}, sp, nil, &fakePoolOps{}, &fakeMultiSig{}, &fakeDexQuoter{}, &fakeSettings{}, "preview", http.NotFoundHandler())
	rec := httptest.NewRecorder()
...
func TestAddWalletRequiresVaultPassword(t *testing.T) {
	st := fakeStatuser{s: supervisor.Status{State: supervisor.StateReady}}
	fv := &fakeVault{exists: true, locked: false}
	h := NewHandler(st, fv, &fakeWallet{}, &fakeSpender{}, nil, &fakePoolOps{}, &fakeMultiSig{}, &fakeDexQuoter{}, &fakeSettings{}, "preview", http.NotFoundHandler())
	rec := httptest.NewRecorder()
...
func TestWalletGatedWhileStarting(t *testing.T) {
	...
	h := NewHandler(st, &fakeVault{}, &fakeWallet{}, &fakeSpender{}, nil, &fakePoolOps{}, &fakeMultiSig{}, &fakeDexQuoter{}, &fakeSettings{}, "preview", http.NotFoundHandler())
	rec := httptest.NewRecorder()
	h.ServeHTTP(rec, httptest.NewRequest(http.MethodGet, "/wallet/balance", nil))
...
func TestSignDataReturnsSignature(t *testing.T) {
	...
	h := NewHandler(fakeStatuser{}, &fakeVault{}, &fakeWallet{}, sp, nil, &fakePoolOps{}, &fakeMultiSig{}, &fakeDexQuoter{}, &fakeSettings{}, "preview", http.NotFoundHandler())
	rec := httptest.NewRecorder()
...
func TestExportUnsignedReturnsTx(t *testing.T) {
	...
	h := NewHandler(st, &fakeVault{}, &fakeWallet{}, sp, nil, &fakePoolOps{}, &fakeMultiSig{}, &fakeDexQuoter{}, &fakeSettings{}, "preview", http.NotFoundHandler())
	rec := httptest.NewRecorder()
	h.ServeHTTP(rec, httptest.NewRequest(http.MethodPost, "/wallet/send/pend1/export-unsigned", nil))
	if rec.Code != http.StatusOK {
...
func TestPoolCredentialsReturnsCreds(t *testing.T) {
	...
	h := NewHandler(fakeStatuser{}, &fakeVault{}, &fakeWallet{}, &fakeSpender{}, nil, po, &fakeMultiSig{}, &fakeDexQuoter{}, &fakeSettings{}, "preview", http.NotFoundHandler())
	rec := httptest.NewRecorder()
...
func TestPoolKESPeriodReady(t *testing.T) {
	...
	h := NewHandler(readyStatuser(), &fakeVault{}, &fakeWallet{}, &fakeSpender{}, nil, po, &fakeMultiSig{}, &fakeDexQuoter{}, &fakeSettings{}, "preview", http.NotFoundHandler())
	rec := httptest.NewRecorder()
	h.ServeHTTP(rec, httptest.NewRequest(http.MethodGet, "/wallet/pool/kes-period", nil))
	if rec.Code != http.StatusOK {
...
// --- Native multi-signature handler tests (from feat/multisig) ---
...
func TestMultiSigListAndCreate(t *testing.T) {
	...
	h := NewHandler(st, &fakeVault{}, &fakeWallet{}, &fakeSpender{}, nil, &fakePoolOps{}, ms, &fakeDexQuoter{}, &fakeSettings{}, "preview", http.NotFoundHandler())
...
func TestMultiSigMyKey(t *testing.T) {
	...
	h := NewHandler(fakeStatuser{}, &fakeVault{}, &fakeWallet{}, &fakeSpender{}, nil, &fakePoolOps{}, ms, &fakeDexQuoter{}, &fakeSettings{}, "preview", http.NotFoundHandler())
	rec := httptest.NewRecorder()
...
// --- Node-local DEX swap handler tests (from feat/dex-swaps) ---
...
func (f *fakeDexQuoter) Quote(_ context.Context, assetIn, assetOut string, amountIn uint64) (dex.Quote, error) {
	f.gotIn, f.gotOut, f.gotAmount = assetIn, assetOut, amountIn
	return f.quote, f.quoteErr
}

func readyWithWallet() (fakeStatuser, *fakeWallet) {
	st := fakeStatuser{s: supervisor.Status{State: supervisor.StateReady}}
	fw := &fakeWallet{set: true} // loaded wallet
	return st, fw
}

func TestDexPoolsReturnsPrices(t *testing.T) {
	st, fw := readyWithWallet()
	dx := &fakeDexQuoter{pools: []dex.Pool{{Protocol: "minswap-v2", PoolID: "p1", PriceXY: 2.0}}}
	h := NewHandler(st, &fakeVault{}, fw, &fakeSpender{}, nil, &fakePoolOps{}, &fakeMultiSig{}, dx, &fakeSettings{}, "preview", http.NotFoundHandler())

	rec := httptest.NewRecorder()
	h.ServeHTTP(rec, httptest.NewRequest(http.MethodGet, "/wallet/dex/pools", nil))
	if rec.Code != http.StatusOK {
		t.Fatalf("GET /wallet/dex/pools = %d, want 200 (body: %s)", rec.Code, rec.Body.String())
	}
	var got struct {
	...
	if len(got.Pools) != 1 || got.Pools[0].Protocol != "minswap-v2" {
		t.Fatalf("pools = %+v, want one minswap-v2 pool", got.Pools)
	}
}

func TestDexPoolsRequiresLoadedWallet(t *testing.T) {
	st := fakeStatuser{s: supervisor.Status{State: supervisor.StateReady}}
	dx := &fakeDexQuoter{}
	h := NewHandler(st, &fakeVault{}, &fakeWallet{}, &fakeSpender{}, nil, &fakePoolOps{}, &fakeMultiSig{}, dx, &fakeSettings{}, "preview", http.NotFoundHandler()) // no wallet

	rec := httptest.NewRecorder()
	h.ServeHTTP(rec, httptest.NewRequest(http.MethodGet, "/wallet/dex/pools", nil))
	if rec.Code != http.StatusConflict {
		t.Fatalf("GET /wallet/dex/pools (no wallet) = %d, want 409", rec.Code)
	}
}

func TestDexPoolsGatedOnNode(t *testing.T) {
	st := fakeStatuser{s: supervisor.Status{State: supervisor.StateStopped}}
	dx := &fakeDexQuoter{}
	h := NewHandler(st, &fakeVault{}, &fakeWallet{set: true}, &fakeSpender{}, nil, &fakePoolOps{}, &fakeMultiSig{}, dx, &fakeSettings{}, "preview", http.NotFoundHandler())

	rec := httptest.NewRecorder()
	h.ServeHTTP(rec, httptest.NewRequest(http.MethodGet, "/wallet/dex/pools", nil))
	if rec.Code != http.StatusServiceUnavailable {
		t.Fatalf("GET /wallet/dex/pools (node stopped) = %d, want 503", rec.Code)
	}
}

func TestDexQuoteSuccess(t *testing.T) {
	st, fw := readyWithWallet()
	dx := &fakeDexQuoter{quote: dex.Quote{
		Protocol: "minswap-v2", PoolID: "p1",
		AmountOut: "1980000", PriceImpactPct: 0.5, EffectiveFee: 0.997,
	}}
	h := NewHandler(st, &fakeVault{}, fw, &fakeSpender{}, nil, &fakePoolOps{}, &fakeMultiSig{}, dx, &fakeSettings{}, "preview", http.NotFoundHandler())

	body := bytes.NewBufferString(`{"asset_in":"lovelace","asset_out":"abcd1234","amount_in":"1000000"}`)
	rec := httptest.NewRecorder()
	h.ServeHTTP(rec, httptest.NewRequest(http.MethodPost, "/wallet/dex/quote", body))
	if rec.Code != http.StatusOK {
		t.Fatalf("POST /wallet/dex/quote = %d, want 200 (body: %s)", rec.Code, rec.Body.String())
	}
	if dx.gotAmount != 1000000 || dx.gotIn != "lovelace" || dx.gotOut != "abcd1234" {
		t.Fatalf("Quote called with (%q,%q,%d), want (lovelace,abcd1234,1000000)", dx.gotIn, dx.gotOut, dx.gotAmount)
	}
	var got dex.Quote
	if err := json.NewDecoder(rec.Body).Decode(&got); err != nil {
		t.Fatalf("decode: %v", err)
	}
	if got.AmountOut != "1980000" {
		t.Fatalf("amount_out = %q, want 1980000", got.AmountOut)
	}
}

func TestDexQuoteBadAmount(t *testing.T) {
	st, fw := readyWithWallet()
	h := NewHandler(st, &fakeVault{}, fw, &fakeSpender{}, nil, &fakePoolOps{}, &fakeMultiSig{}, &fakeDexQuoter{}, &fakeSettings{}, "preview", http.NotFoundHandler())

	body := bytes.NewBufferString(`{"asset_in":"lovelace","asset_out":"abcd","amount_in":"not-a-number"}`)
	rec := httptest.NewRecorder()
	h.ServeHTTP(rec, httptest.NewRequest(http.MethodPost, "/wallet/dex/quote", body))
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("POST /wallet/dex/quote (bad amount) = %d, want 400", rec.Code)
	}
}
...
func TestDexQuoteErrorMapping(t *testing.T) {
	cases := []struct {
		...
		{"invalid", dex.ErrInvalidRequest, http.StatusBadRequest},
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			body := bytes.NewBufferString(`{"asset_in":"lovelace","asset_out":"abcd","amount_in":"1000000"}`)
			rec := httptest.NewRecorder()
			h.ServeHTTP(rec, httptest.NewRequest(http.MethodPost, "/wallet/dex/quote", body))
			if rec.Code != tc.wantCode {
				t.Fatalf("%s: code = %d, want %d", tc.name, rec.Code, tc.wantCode)
...
func TestDexEndpointsAbsentWhenNil(t *testing.T) {
	// With dx==nil the DEX routes are not registered; the SPA catch-all (here a
	// 404 handler) serves them instead.
	st, fw := readyWithWallet()
	h := NewHandler(st, &fakeVault{}, fw, &fakeSpender{}, nil, &fakePoolOps{}, &fakeMultiSig{}, nil, &fakeSettings{}, "preview", http.NotFoundHandler())
	rec := httptest.NewRecorder()
	h.ServeHTTP(rec, httptest.NewRequest(http.MethodGet, "/wallet/dex/pools", nil))
	if rec.Code != http.StatusNotFound {
		t.Fatalf("GET /wallet/dex/pools (nil dex) = %d, want 404", rec.Code)
	}
}

// --- App settings (lean-node history-expiry) handler tests (from feat/mobile) ---
...
func TestGetHistoryExpiryReturnsState(t *testing.T) {
	...
	h := NewHandler(st, &fakeVault{}, &fakeWallet{}, &fakeSpender{}, nil, &fakePoolOps{}, &fakeMultiSig{}, &fakeDexQuoter{}, set, "preview", http.NotFoundHandler())
	rec := httptest.NewRecorder()
