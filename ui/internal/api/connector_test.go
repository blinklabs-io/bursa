...
// TestConnectorSelfRevoke tests POST /connector/self-revoke: the
// extension-facing, token-gated counterpart of the SPA-facing
// /connector/grants/revoke. The paired extension's popup has no way to
// satisfy strictSameOrigin (its Origin is chrome-extension://<id>, not the
// API's own host), so it must be able to revoke a grant using its bearer
// token instead.
func TestConnectorSelfRevoke(t *testing.T) {
	const extID = "chrome-extension://selfrevoketest"
	...
	postSelfRevoke := func(t *testing.T, srv *httptest.Server, token, body string) *http.Response {
		t.Helper()
		req, err := http.NewRequest(http.MethodPost, srv.URL+"/connector/self-revoke", strings.NewReader(body))
		if err != nil {
			t.Fatalf("NewRequest: %v", err)
		}
		req.Header.Set("Content-Type", "application/json")
	...
	t.Run("valid token revokes a granted origin", func(t *testing.T) {
		svc, srv, token := newSvcServer(t)
		...
		go func() {
			defer close(done)
			//nolint:errcheck
			svc.Handle(context.Background(), "https://selfrevoke.io", "enable", nil)
		}()
		decideWhenPending(t, svc, connector.Decision{Approved: true})
		<-done
		...
		resp := postSelfRevoke(t, srv, token, `{"origin":"https://selfrevoke.io"}`)
		defer resp.Body.Close()
		...
		for _, g := range svc.Grants() {
			if g == "https://selfrevoke.io" {
				t.Fatalf("origin still granted after self-revoke: %v", svc.Grants())
			}
		}
	})

	t.Run("missing/invalid token → 401, grant untouched", func(t *testing.T) {
		svc, srv, _ := newSvcServer(t)

		done := make(chan struct{})
		go func() {
			defer close(done)
			//nolint:errcheck
			svc.Handle(context.Background(), "https://stillgranted.io", "enable", nil)
		}()
		decideWhenPending(t, svc, connector.Decision{Approved: true})
		<-done

		resp := postSelfRevoke(t, srv, "wrong-token", `{"origin":"https://stillgranted.io"}`)
		defer resp.Body.Close()
		...
		found := false
		for _, g := range svc.Grants() {
			if g == "https://stillgranted.io" {
				found = true
			}
		}
		if !found {
			t.Error("grant was removed despite an invalid token")
		}
	})

	t.Run("empty origin → 400", func(t *testing.T) {
		_, srv, token := newSvcServer(t)

		resp := postSelfRevoke(t, srv, token, `{"origin":""}`)
		defer resp.Body.Close()
		if resp.StatusCode != http.StatusBadRequest {
			t.Fatalf("status = %d, want 400", resp.StatusCode)
		}
