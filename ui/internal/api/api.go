...
// confirming it (decrypt → sign → submit), signing messages, and building
// staking/governance delegation transactions (which Confirm signs + submits
// through the same path as a send).
...
// lookup verifies pasted pool/DRep IDs through the node (may be nil, in which
// case those endpoints report unavailable). spa is the handler for the embedded
// SPA; it is registered as the catch-all route so that the specific API routes
// above take precedence on the mux.
func NewHandler(st Statuser, wl Wallet, sp Spender, lookup PoolDRepLookup, network string, spa http.Handler) http.Handler {
