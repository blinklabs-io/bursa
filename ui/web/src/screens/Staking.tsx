...
function Compose(props: ComposeProps) {
  ...
  async function handleReview() {
    ...
    if (voteType) {
      const vote: DelegationVote = { type: voteType };
      if (voteType === "drep") {
        if (!drepId.trim()) {
          setError("Enter a DRep ID to delegate your vote to.");
          return;
        }
        vote.drep_id = drepId.trim();
      }
      if (voteType === "register_self" && anchorUrl.trim()) {
        vote.anchor = { url: anchorUrl.trim(), hash: anchorHash.trim() };
      }
      req.vote = vote;
    }
                ...
                {selected && opt.type === "register_self" && (
                  <div className="vote-opt-extra">
                    ...
                    <p className="deposit-note">
                      Registering as a DRep locks a <strong>500 ₳ deposit</strong> (a
                      protocol parameter read from your node, refundable when you
                      retire). This is separate from the 2 ₳ stake-key deposit.
                    </p>
                  </div>
                )}
              </div>
...
export function Staking() {
  ...
  function handlePreview(p: DelegationPreview) {
    setPreview(p);
    setPhase("preview");
  }
  ...
  if (phase === "preview" && preview) {
    return (
      <PreviewPhase
        preview={preview}
        onBack={() => setPhase(isActive ? "status" : "compose")}
        onDone={handleDone}
      />
    );
  }
