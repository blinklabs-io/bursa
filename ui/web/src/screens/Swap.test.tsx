...
function stubPools(pools: DexPool[], opts?: { error?: Error | null; loading?: boolean }) {
...
test("formats very large / very small pool prices without scientific notation", () => {
  // A pool pairing assets with wildly different decimals can produce a
  // price outside toPrecision's fixed-notation range (roughly 1e-6..1e21),
  // where it would otherwise render as e.g. "1.23457e-7".
  const tinyPrice: DexPool = { ...POOL, pool_id: "p-tiny", price_xy: 0.00000012345 };
  const hugePrice: DexPool = { ...POOL, pool_id: "p-huge", price_xy: 123456789 };
  stubPools([tinyPrice, hugePrice]);
  render(<Swap />);

  expect(screen.queryByText(/e[+-]\d/)).not.toBeInTheDocument();
  expect(screen.getByText("0.00000012345")).toBeInTheDocument();
  expect(screen.getByText("123457000")).toBeInTheDocument();
});

test("keeps stale pool data visible when refresh fails", () => {
  stubPools([POOL], { error: new Error("temporary network error") });
  render(<Swap />);

  expect(screen.getByText("minswap-v2")).toBeInTheDocument();
  expect(screen.getByRole("alert")).toHaveTextContent(/could not refresh pools/i);
  expect(screen.getByRole("alert")).toHaveTextContent(/temporary network error/i);
});

test("gets the best quote and shows route, amount out, price impact and fee", async () => {
  stubPools([POOL]);
  const computeDexQuote = vi.spyOn(client, "computeDexQuote").mockResolvedValue(QUOTE);
  ...
  await waitFor(() =>
    expect(computeDexQuote).toHaveBeenCalledWith({
      asset_in: "lovelace",
      asset_out: "abcd1234",
      amount_in: "1000000",
    }),
...
test("surfaces the API error when no route is found", async () => {
  stubPools([POOL]);
  vi.spyOn(client, "computeDexQuote").mockRejectedValue(
