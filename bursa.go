...
// GetScriptType returns the script type identifier
...
func NewTimelockedScript(
	...
	return NewScriptAll(timelockScript, script)
}
...
func validateScriptNOfWithDepth(
	...
	requiredSigs := int(script.N)
	if requireSignatures && len(signatures) < requiredSigs {
		return false
	}
	satisfied := 0
	for _, subScript := range script.Scripts {
