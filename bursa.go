...
type WalletConfig struct {
	...
	Password        string //nolint:gosec // G117: not a hardcoded secret
...
func GetVRFSeed(rootKey bip32.XPrv, index uint32) ([]byte, error) {
	...
	indexBytes[0] = byte(index >> 24) //nolint:gosec // G115: intentional byte extraction
	indexBytes[1] = byte(index >> 16) //nolint:gosec // G115: intentional byte extraction
	indexBytes[2] = byte(index >> 8)  //nolint:gosec // G115: intentional byte extraction
	indexBytes[3] = byte(index)       //nolint:gosec // G115: intentional byte extraction
